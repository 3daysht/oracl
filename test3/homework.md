...
在主表orders和从表order_details之间建立引用分区在study用户中创建两个表：orders（订单表）和order_details（订单详表），
...
## 分配指定权限：
```
grant select on ORDERS to wyview;
grant select on ORDER_DETAILS to wyview;
grant UNLIMITED TABLESPACE to wy;
```
